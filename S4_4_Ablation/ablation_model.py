...
class DCGAN:
    ...
    def train(self, epochs, X_train, batch_size=128, save_interval=50, save=False, save_model_interval=100):
        ...
        for epoch in range(epochs):
            ...
            if save and epoch > stop:
                if os.path.isdir('ablation_model/') != True:
                    os.mkdir('ablation_model/')
                if (epoch % save_model_interval == 0 and epoch > 0):
                    self.discrimintor.save('ablation_model/%d.h5' % epoch)
    ...
    def save_image(self, epoch):
        if os.path.isdir('ablation_ecg_image/') != True:
            os.mkdir('ablation_ecg_image/')
        r, c = 2, 2
        ...
        for i in range(r):
            for j in range(c):
                ...
                cnt += 1
        fig.savefig('ablation_ecg_image/%d.png' % epoch)
        plt.close()
